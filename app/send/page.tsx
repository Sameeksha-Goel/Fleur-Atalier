...
import { BouquetState, decodeBouquetUrl } from "@/lib/bouquetState";
...
export default function SendPage() {
  ...
  useEffect(() => {
    ...
    if (!encoded) { setState("not-found"); return; }
    const decoded = decodeBouquetUrl(encoded, "shared");
    setState(decoded ?? "not-found");
  }, []);

  if (state === "loading") return <div className="min-h-screen bg-cream" />;
  ...
  if (state === "not-found") {
    return (
      <main className="min-h-screen bg-cream flex flex-col items-center justify-center px-6 text-center gap-5">
        ...
        <Link href="/create" className="font-sans text-sm text-terracotta hover:underline underline-offset-4">
          Build your own →
        </Link>
      </main>
    );
  ...
  const bouquet = state;
  const hasLetter = bouquet.letter.to.trim() || bouquet.letter.message.trim() || bouquet.letter.from.trim();
